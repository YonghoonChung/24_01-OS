// On May 11th, we had an online zoom meeting to review the midterm problems.
// We found that my code had no bug!. I did not change anything.
//
#include <stdio.h>
#include <stdlib.h>
...
void Shoot(int sx, int sy, int width)
{
	...
	if(param == NULL){
		printf("Failed to dynamically allocate the MissileInfo");
		return;
	}	
	param->sx = sx;
	param->sy = sy;
